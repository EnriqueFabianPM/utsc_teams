...
class _UsuariosCrudState extends State<UsuariosCrud> {
  ...
  Future<void> _openForm({Usuario? u}) async {
                ...
                children: [
                  ...
                  TextField(
                    ...
                    decoration: InputDecoration(
                      labelText: u == null ? 'Contraseña' : 'Contraseña (vacío = conservar)',
                      suffixIcon: IconButton(
                        icon: Icon(hidePass ? Icons.visibility : Icons.visibility_off),
                        onPressed: () => setModal(() => hidePass = !hidePass),
                      ),
                    ),
                  ),
                  const SizedBox(height: 12),
                  DropdownButtonFormField<String>(
                    ...
                    onChanged: (v) => setModal(() => rol = v ?? 'estudiante'),
                  ...
                  if (rol == 'estudiante')
                    DropdownButtonFormField<int>(
                      value: grupoId,
                      items: _grupos.map((g) => DropdownMenuItem<int>(
                        value: g['id'] as int,
                        child: Text(_grupoNombreById[g['id'] as int] ?? g['nombre'].toString()),
                      )).toList(),
    ...
    if (nombre.isEmpty) error = 'Nombre requerido.';
    else if (!emailRegex.hasMatch(email)) error = 'Email inválido.';
    else if (u == null && pass.isEmpty) error = 'Contraseña requerida.';
    else if (rol == 'estudiante' && grupoId == null) error = 'Selecciona un grupo.';
    ...
    try {
      if (u == null) {
        await db.createUsuario(Usuario(
          id: null, nombre: nombre, email: email, password: pass,
          rol: rol, grupoId: rol == 'estudiante' ? grupoId : null,
        ));
      } else {
        await db.updateUsuario(Usuario(
          id: u.id, nombre: nombre, email: email,
  ...
  Widget _roleFilterChips() {
    return Wrap(
      spacing: 8,
      children: [
        ChoiceChip(label: const Text('Todos'), selected: _rolFilter == null, onSelected: (_) { _rolFilter = null; _reload(); }),
        for (final r in _roles)
          ChoiceChip(label: Text(r), selected: _rolFilter == r, onSelected: (_) { _rolFilter = r; _reload(); }),
      ],
    );
  }
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          Padding(padding: const EdgeInsets.symmetric(horizontal: 16), child: _roleFilterChips()),
          ...
          Expanded(
            child: FutureBuilder<List<Usuario>>(
              ...
              builder: (_, snap) {
                ...
                if (data.isEmpty) return const Center(child: Text('Sin usuarios'));
                  ...
                  itemBuilder: (_, i) {
                    ...
                    return ListTile(
                      ...
                      trailing: Wrap(
                        spacing: 8,
                        children: [
                          IconButton(icon: const Icon(Icons.edit), onPressed: () => _openForm(u: u)),
