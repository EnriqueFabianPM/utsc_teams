...
class _CalificacionesMaestroState extends State<CalificacionesMaestro>
  ...
  Future<void> _editar(Trabajo t) async {
    ...
    // Captura solo lo que sí usaremos tras el await
        ...
        content: Column(
          ...
          children: [
            TextField(
              ...
              decoration:
              const InputDecoration(labelText: 'Calificación (0–100)'),
    ...
    if (!mounted) return; // evita usar context si la vista se cerró
    setState(() {}); // refresca
    messenger.showSnackBar(
  ...
  Widget _list(Future<List<Trabajo>> future) {
    return FutureBuilder<List<Trabajo>>(
      ...
      builder: (_, snap) {
        ...
        final items = snap.data ?? const <Trabajo>[];
        ...
        return ListView.separated(
          ...
          itemBuilder: (_, i) {
            ...
            final califColor = t.calificacion == null ? Colors.grey : Colors.green;

            // Si estudianteId es no nulo en tu modelo, no uses ?.
            final alumno = t.estudianteNombre ?? t.estudianteId.toString();

            return ListTile(
              title: Text(t.titulo),
              subtitle: Text('Alumno: $alumno • Grupo ${t.grupoId}'),
              trailing: Column(
